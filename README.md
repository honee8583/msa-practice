...
Spring Cloud Gateway는 마이크로서비스 아키텍처(MSA)에서 외부 요청을 받아 적절한 내부 서비스로 라우팅하는 **API 게이트웨이** 역할을 하는 프로젝트입니다. Netflix Zuul의 대체제로 개발되었으며, 논블로킹(Non-Blocking) I/O 모델을 기반으로 높은 성능을 제공합니다.
